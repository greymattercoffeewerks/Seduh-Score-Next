...
_Status: DRAFT for owner review. Nothing here is published. Claims marked [VERIFY] depend on
platform behaviour that must be confirmed or built first (see ROADMAP.md, Trust and
transparency)._
...
- **The rules are fixed code, and public.** Ranking, tie-breaks and advancement are
  deterministic and the source code is public, so anyone can read exactly how a result is
  worked out.
- **Results can be re-derived.** Standings are calculated from the raw per-cupper scores
  every time; they are never typed in as a final figure.
- **Changes leave a trail.** [VERIFY: only after T-TRUST.1 ships] Every score or time edit
  is logged with who, when, the old and new value, and a reason for corrections after
