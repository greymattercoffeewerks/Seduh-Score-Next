...
| v5.0    | **Menglait** | ⚠️ **BTC actually shipped 2026-09-23, as the platform's SECOND real format — ahead of Throwdown and Liga Seduh, not the 4th as this row assumed.** BTC = Barista Team Championship (renamed from BBTC at the start of Phase T-BTC.2, 2026-09-18 — BBTC is the Brunei-specific instance, not the format's own name). This table hasn't been renumbered to reflect that out-of-order ship; flagged here rather than silently left wrong. | Legacy's own Menglait was a stress-test milestone (Girls Got Drip Vol. 0), not a format ship — the thematic fit is weaker here, flagged rather than forced. Revisit this pairing (and the v3.0/v4.0 rows above, now that BTC beat both to actually shipping) against the real build order. |
