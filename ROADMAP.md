...
| Phase 4 — Cup Taster                | 🚧 In progress (T4.1–T4.3 done) | Setup, heat generation, timing (app + manual), scoring, standings/advancement, report, export |
...
- **T4.3's `timing.js` writes directly to the database, not through Phase 3's outbox.**
  Decided with the user before writing code (see CHANGELOG.md) — this is the first
  genuinely live, time-pressured screen in the build plan, exactly what the outbox exists
  for, but wiring it correctly (queue ordering against a live countdown, conflict
  surfacing to an organiser mid-heat) deserves a focused pass with `offline-sync-auditor`
  rather than folding into this task's first cut. Revisit before relying on this screen
  somewhere with unreliable connectivity.
